...
like `copyToZeroE` over parameterized versions like `copyToZeroP` that
...
`xs.T`, the opposite is not true;
`mdropFirstT` <*<sub><small>m</small></sub>* `mdropFirstE`, or,
...
that `mdropFirstT` <*<sub><small>m</small></sub>* `mdropFirstE`, and
therefore, that you can’t get away with simply dropping the
refinements.
...
*m1* ≡ *m2* implies *m1* ≡*<sub><small>m</small></sub>* *m2*, and
*m3* <: *m4* implies *m3* <:*<sub><small>m</small></sub>* *m4*.
...
*t1* <: *t2*, so *r* ≡*<sub><small>m</small></sub>* *q* implies
...
of `mdropFirstT` <:*<sub><small>m</small></sub>* `mdropFirstE`
justifies my choice.  `mdropFirstT` has more specific type, and
rejects more implementations, such as the one that returns a list with
`42` in it above.  Thus, it has fewer implementations, in the same way
that more specific types have fewer values inhabiting them.  But it
can be used in more circumstances, so it is “more general”.  The
generality in terms of when a method can be used is directly
proportional to the specificity of its type.
